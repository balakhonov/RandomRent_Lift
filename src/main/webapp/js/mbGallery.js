...
(function($){
	...
	$.mbGallery ={
		...
		buildMbGallery:function(options){
			...
			if (gallery.options.printOutThumbs){// &&
												// gallery.options.containment=="body"
		...
		getPhotos: function(){
			...
			$(gallery).find(gallery.options.thumbnailSelector).each(function(i){
				...
				if(gallery.options.showDescription)
      ...
      galleryDesc.html(gallery.images[gallery.idx].description);
		...
		showExifData:function(){
			var gallery= $(this).get(0);
			/*
			 * EXIF methods: $(this).exif(key): a specific key;
			 * $(this).exifPretty(): all key as string; $(this).exifAll(): all
			 * key as object;
			 */
...
// $("#"+gallery.options.containment).slideUp();
	...
	jQuery.fn.extend({
		getDim:function(gallery,w,h){
			...
			/*
			 * PHB if (parseFloat(nh)+120>=wh || gallery.options.fullScreen){
			 * nh= wh-130; nw=(nh*$(this).outerWidth())/$(this).outerHeight();
			 * $(this).attr("height", nh); $(this).attr("width", nw); } if
			 * (parseFloat(nw)+100>=ww ){ nw= ww-120;
			 * nh=(nw*$(this).outerHeight())/$(this).outerWidth();
			 * $(this).attr("width", nw); $(this).attr("height", nh); }
			 */

			// If either dimension of the image is too large for the window or
	...
	$("body").on("contextmenu", ".galleryScreen", function() {
		return false;
	})
})(jQuery);
